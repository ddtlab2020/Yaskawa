...
class qtWindowComic(QWidget):
    ...
    def textToSVG(self,xTranslate,yTranslate, text, widthText, z):
        print(text)
        #global lines
        textAr = []
        ...
        with cairo.SVGSurface(self.folderPath+"outputTextPath.svg", 700, 700) as surface:
        ...
        doc = minidom.parse(self.folderPath+"outputTextPath.svg")  # parseString also exists
    ...
    def drawComicText(self, event, qp):
        ...
        #qp = self.textCursor()
        #qp.movePosition(QTextCursor.Start)
 
        #my_char_format = qp.charFormat()
        #qp.setCharFormat(my_char_format)
 
        #qp.insertText("Text example", my_char_format)
 
        #outline_format = my_char_format
        #outline_format.setTextOutline(QPen(Qt.red, 5))
 
        #qp.setCharFormat(outline_format)
        #qp.insertBlock()
        #qp.insertText("Text example")
